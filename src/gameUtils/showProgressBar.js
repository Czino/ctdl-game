...
export const showProgressBar = progress => {
  let queueBrightness = 1 - Math.abs(Math.sin(CTDLGAME.frame / 36) * .1)
  let blocksInQueue = Math.ceil((1 - progress) * 3)
  ...
  let offset = Math.round((progress * 100) % 33.33)
  let offsetSlow = Math.min(8, offset)
  let offsetFast = Math.min(14, offset)
  for (let b = 0; b < blocksInQueue; b++) {
    let x = constants.WIDTH / 2 - (8 * (b + 1)) - 2
    x += b === 0 ? offsetFast : offsetSlow
    let y = constants.HEIGHT / 2 - 12
    renderBlock(
  ...
  for (let b = 0; b < 3; b++) {
    let x = constants.WIDTH / 2 + (8 * (b + 1)) - 4 + offsetSlow
    let y = constants.HEIGHT / 2 - 12
