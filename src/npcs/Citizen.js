...
// TODO only "protest while condition is met"
// TODO otherwise just walk around
...
class Citizen extends Agent {
  constructor(id, options) {
    super(id, options)
    this.spriteData = sprites[random(Object.keys(sprites))]
    this.maxHealth = options.maxHealth ?? 5
    this.health = options.health ?? 5
    this.strength = 1
    this.attackRange = Math.ceil(Math.random() * 10)
    this.senseRadius = 50
    this.walkingSpeed = options.walkingSpeed || 3
    this.runSpeed = options.runSpeed || Math.round(Math.random() * 2) + 4
    this.protection = 0
  }
